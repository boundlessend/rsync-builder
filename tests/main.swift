...
// runEnvironment: пустой askpass - PATH и RSYNC_RSH заданы, проводки askpass нет
let envNoPass = runEnvironment(base: [:], rsh: "ssh -o X", askpass: "")
assert(envNoPass["RSYNC_RSH"] == "ssh -o X", envNoPass["RSYNC_RSH"] ?? "nil")
assert(envNoPass["PATH"]?.contains("/usr/bin") == true, "PATH not set")
assert(envNoPass["SSH_ASKPASS"] == nil, "askpass leaked without a password")

// runEnvironment: непустой askpass - проводка выставлена; пароль в окружение НЕ кладётся
let envPass = runEnvironment(base: [:], rsh: "ssh", askpass: "/tmp/a.sh")
...
assert(envPass["RSYNC_BUILDER_PASS"] == nil, "password must not be in the environment")

// askpassScript: путь заквочен, читается через cat
assert(askpassScript(passwordFile: "/tmp/x y.pw").contains("cat '/tmp/x y.pw'"), askpassScript(passwordFile: "/tmp/x y.pw"))
