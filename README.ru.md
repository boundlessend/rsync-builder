...
- Импорт серверов из `~/.ssh/config` (тот же файл, что использует VS Code Remote-SSH) через меню «•••»
...
- Необязательное поле пароля SSH - запуск без терминала; пусто = вход по ключу.
  Пароль держится в памяти и передаётся ssh через временный 0600-файл, существующий только
  на время запуска. Host key незнакомого сервера при первом подключении принимается
  автоматически (`StrictHostKeyChecking=accept-new`); для ручной проверки есть «Запустить в терминале»
...
- Язык интерфейса по умолчанию берётся из системы (English / Русский), переключается в Настройках
